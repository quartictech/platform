...
class WebsocketClient<in TSend, out TReceive>(
    uri: URI,
    private val receiveClass: Class<TReceive>
) {
    private val _toServer = Channel<TSend>(UNLIMITED)
    private val _fromServer = Channel<TReceive>(UNLIMITED)
    val toServer: SendChannel<TSend> = _toServer
    val fromServer: ReceiveChannel<TReceive> = _fromServer
    ...
    companion object {
        inline fun <TSend, reified TReceive> create(uri: URI): WebsocketClient<TSend, TReceive> =
            WebsocketClient(uri, TReceive::class.java)
    }
}
